...
curtime=$(TZ=GMT-7 date '+%H:%M')
if [ "${curtime}" != "${PHASE1}" ] && [ "${curtime}" != "${PHASE2}" ]; then
    sleep 1
    continue
fi
echo " "
...
##
cloudflareUrl=$(curl -s -X POST -H "Content-Type: application/json" -H "X-Auth-Email: $cloudflareemail" -H  "Authorization: Bearer $cloudflareapikey" --data "$(echo $PAYLOAD)" https://api.cloudflare.com/client/v4/graphql/ 2>&1 -k --silent)
